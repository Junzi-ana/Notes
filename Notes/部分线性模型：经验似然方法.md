...
考虑到在 Wilks 定理中，$-2\ln R(\theta)$ 有 $U_{Z}^{T}S^{-1}_{Z}U_{Z}+o_{p}(1)$ 的形式，其中
$$
U_{Z}=\frac{1}{\sqrt{ n }}\sum^{n}_{i=1}Z_{i}(\theta),\quad S_{Z} =\frac{1}{n}\sum^{n}_{i=1}Z_{i}(\theta)Z_{i}(\theta) ^{T}
$$
分别是经验得分和经验信息阵。于是我们只需要证明 $U_{\widehat{Z}}=U_{Z}+o_{p}(1)$ 和 $S_{\widehat{Z}}=S_{Z}+o_{p}(1)$ 。为此，令估计余项为
$$
r_{i}=\widehat{Z}_{i}(\theta)-Z_{i}(\theta)
$$
要证
明 $\frac{1}{\sqrt{ n }}\sum^{n}_{i=1}r_{i}=o_{p}(1)$ ，一般来说可以利用 Chebyshev 不等式。假如估计得当，$\mathbb{E}r_{i}=0$ 且 $i\neq j$ 时 $\mathbb{E}r_{i}r_{j}=0$，就有
$$
