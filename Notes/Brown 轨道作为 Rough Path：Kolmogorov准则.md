...
很容易验证 $\mathbf{B}(\omega)$ 满足 Chen's Relation，然而要说明其轨道的 Hölder 连续性，却并不直接，要依赖如下定理。这个定理的形式与证明过程基本类似于 Kolmogorov-Čentsov 定理，*参见 Shreve 第二章定理 2.8*，因此得名 **Kolmogorov Criterion** 。
...
$$
此外，$K_{\alpha}(\omega),~\mathbb{K}_{\alpha}(\omega)$ 视为随机变量是 $L^{q},~L^{\frac{q}{2}}$ 中的元素。特别地，若 $\beta-\dfrac{1}{q}>\dfrac{1}{3}$，则 $\boldsymbol{X}=(X,~\mathbb{X})\in \mathcal{C}^{\alpha},~~\text{a.s.}$
