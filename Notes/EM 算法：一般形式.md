...
$$
\begin{align}
\mathscr{l}(x|\pi,\mu,\Sigma)&=\sum^{n}_{i=1} \sum^{K}_{k=1}\gamma_{ik}(\ln \pi_{k}+\ln p(x_{i}|\mu_{k},\Sigma_{k})) \\
&=\mathbb{E}_{z}\mathscr{l}(x,z|\pi,\mu,\Sigma)
\end{align}
...
$$
\mathscr{l}(X|\theta)=\mathbb{E}_{Z}\big[ \mathscr{l}(X,Z|\theta)\,\big|X,\theta ] 
$$

于是 EM 算法可以被总结如下：
1. 确定迭代初值 $\theta^{(0)}$
2. **(Expectation)** 计算 $Q(\theta;\theta^{(n)})=\mathbb{E}_{Z}\big[ \mathscr{l}(X, Z|\theta)\,\big|X,\theta^{(n)} ]$
